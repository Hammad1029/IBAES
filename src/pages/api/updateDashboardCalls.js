...
export default async (req, res) => {
  try {
    if (req.method === "GET") {
      // if (!verifyAuth(req)) return responseHandler(res, responses.unauthorized, { error: true });
      const modules = await sheets.readGoogleSheet(sheets.tabs.modules);
      const users = await sheets.readGoogleSheet(sheets.tabs.users);
      const teamScores = users.map(({ id, username, members, score }) => ({
        id,
        username,
        members,
        score,
